...
export class ServiceWorkerService {
    ...
    constructor(private snackBarService: SnackBarService,  private sw: NgServiceWorker,  private http: Http, @Inject(PLATFORM_ID)  platformId: Object, private appRef: ApplicationRef) {
        this.platformId = platformId;
        if (this.isPushAvailable())
            this.isRegisteredToPushObs.next(this.isRegistered());
        if ( !isPlatformBrowser(this.platformId) || !(process.env.NODE_ENV == 'production' && 'serviceWorker' in navigator) || localStorage.getItem("cache_done") == "true")
            return;
    ...
    public registerToPush():void {
        this.sw.registerForPush({
            ...
            this.http.post("https://api.angular-universal-serverless.maciejtreder.com/webpush/vapid/subscribe", JSON.stringify(pushRegistration), options)
        ...
        if (window['safari'] && window['safari'].pushNotification) {
            window['safari'].pushNotification.requestPermission(
                "https://api.angular-universal-serverless.maciejtreder.com/webpush/safari",
                "web.com.maciejtreder.angular-universal-serverless",
                null,
                (permission) => {
            ...
            this.sw.registerForPush({
                ...
                subscription.unsubscribe().subscribe(result => {
                    if (result) {
                        ...
                        headers.append("content-type", "application/json");
                        let options: RequestOptionsArgs = new RequestOptions({headers: headers});
                        this.http.post("https://api.angular-universal-serverless.maciejtreder.com/webpush/vapid/unsubscribe", JSON.stringify(subscription), options).subscribe((res: Response) => {
                            if (res.status == 202) {
